    ...
    card.innerHTML = `
        <div class="list-card-header">
            ...
            <div class="list-card-actions">
                <button class="list-action-btn" onclick="window.editTaskList(${list.id})" title="Editar">
                    ✏️
                </button>
                <button class="list-action-btn danger" onclick="window.deleteTaskList(${list.id})" title="Eliminar">
                    🗑️
                </button>
            </div>
        </div>
...
export function updateSidebarLists() {
    ...
    const existingLists = listsSection.querySelectorAll('.sidebar-item[data-list-id]');
...
export async function submitTaskList(listData) {
    ...
    const url = isEdit 
        ? `http://localhost:3000/api/task-lists/${listData.id}`
...
export async function deleteTaskListById(listId) {
    if (!confirm('¿Estás seguro de que quieres eliminar esta lista? Todas las tareas asociadas también se eliminarán.')) {
        return;
    }

    try {
        const response = await fetch(`http://localhost:3000/api/task-lists/${listId}`, {
            method: 'DELETE',
            credentials: 'include'
        });
